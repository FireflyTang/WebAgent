...
WebAgent is a trusted, local demonstration project. The administrator-created
name gate and Session owner field only separate the product UI; names are not
credentials, administrator APIs are open, and the routes do not implement an
authorization boundary. Run it only on a machine and network
